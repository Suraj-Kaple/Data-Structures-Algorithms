...
void minHeapify(vector<int> &A, int i, int n){
    ...
    // find minimum among the three nodes
    if(r<=n-1){
        if(A[i] > A[r]) swap(A[i], A[r]);
    }
    
    if(A[i] > A[l]) swap(A[i], A[l]);
    // call minHeapify on child nodes
    minHeapify(A, l, n);
    if(r<=n-1)  minHeapify(A, r, n);    
}

void buildHeapTillB(vector<int> &A, int B){     // runs in O(B) (might seem Blog(B))
    int start = ceil(B/2) - 1;  // start from the bottom-right-most non leaf node
